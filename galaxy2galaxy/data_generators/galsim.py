...
@registry.register_problem
class GalsimCosmosConditional(GalsimCosmos):
  ...
  def generator(self, tmp_dir, task_id=-1):
    ...
    for ind in index:
      ...
      parameters = {k:params[k] for k in  p.cosmos_parameters}
...
@registry.register_problem
class GalsimCosmos32Conditional(GalsimCosmosConditional):

  def hparams(self, defaults, model_hparams):
    p = defaults
    p.modality = {"inputs": modalities.ModalityType.IDENTITY,
                  "conditions": modalities.ModalityType.IDENTITY,
                  "targets": modalities.ModalityType.IDENTITY}
    p.vocab_size = {"inputs": None,
                    "conditions": None,
                    "targets": None}
    ...
    p.img_len = 32
    p.example_per_shard = 1000
    p.cosmos_parameters = ['mag_auto', 'flux_radius', 'zphot', 'bulge_q', 'bulge_beta' ,
